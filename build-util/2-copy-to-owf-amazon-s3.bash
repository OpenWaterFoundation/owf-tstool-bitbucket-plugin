...
printUsage() {
  ...
  echoStderr ""
  echoStderr "Copy the TSTool Bitbucket plugin installer files to the Amazon S3 static website folder:"
...
uploadInstaller() {
  ...
  installerUploadCount=0
    ...
    if [ "$includeWindows" = "yes" ]; then
      ...
      zipFilePattern="${distFolder}/tstool-bitbucket-plugin-${pluginVersion}-win-*.zip"
      ...
      if [ -f "${latestZipFile}" ]; then
        ...
        if [ ${errorCode} -ne 0 ]; then
          ...
          cloudFrontFile="/tstool-bitbucket-plugin/${pluginVersion}/software/*"
...
logInfo ""
logInfo "${installerUpdateCount} TSTool Bitbucket plugin installers were uploaded to Amazon S3 location:"
logInfo ""
logInfo "  ${s3FolderUrl}"
logInfo "  ${gpDownloadUrl}"
logInfo ""
logInfo "Next steps are to do the following:"
logInfo "  - Visit the following folder to download the TSTool Bitbucket plugin:"
logInfo "      ${gpDownloadUrl}/"
logInfo ""
