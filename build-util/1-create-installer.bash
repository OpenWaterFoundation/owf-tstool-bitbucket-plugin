...
#     tstool-bitbucket-plugin-1.0.0-win-2206060102.zip
...
createPluginZipFile() {
  ...
  "${sevenzip}" a -tzip ${zipFile} "owf-tstool-bitbucket-plugin/${pluginVersion}"
  exitStatus=$?
...
devBinFolder="${repoFolder}/owf-tstool-bitbucket-plugin/target/classes"
...
mainPluginFolder="${pluginsFolder}/owf-tstool-bitbucket-plugin"

# Version installed folder for the plugin.
versionPluginFolder="${mainPluginFolder}/${pluginVersion}"

# Jar file for the plugin.
jarFile="${versionPluginFolder}/owf-tstool-bitbucket-plugin-${pluginVersion}.jar"

# Folder for dependencies.
pluginDepFolder="${versionPluginsFolder}/dep"

now=$(date +%Y%m%d%H%M)
zipFile="${distFolder}/tstool-bitbucket-plugin-${pluginVersion}-win-${now}.zip"
