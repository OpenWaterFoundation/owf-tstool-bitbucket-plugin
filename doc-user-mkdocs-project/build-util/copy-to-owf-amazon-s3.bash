...
getPluginVersion() {
  ...
  # Maven folder structure results in duplicate 'owf-tstool-bitbucket-plugin'?
  # TODO smalers 2022-05-19 need to enable this.
  srcFile="${repoFolder}/owf-tstool-bitbucket-plugin/src/main/java/org/openwaterfoundation/tstool/plugin/bitbucket/PluginMeta.java"  
...
dryrun=""
#dryrun="--dryrun"
s3VersionFolder="s3://software.openwaterfoundation.org/tstool-bitbucket-plugin/${pluginVersion}/doc-user"
s3LatestFolder="s3://software.openwaterfoundation.org/tstool-bitbucket-plugin/latest/doc-user"
...
if [ -n "${pluginVersion}" ]; then
  ...
  ${awsExe} cloudfront create-invalidation --distribution-id ${cloudFrontDistributionId} --paths "/tstool-bitbucket-plugin/${pluginVersion}/doc-user/*" --output json --profile "${awsProfile}" | tee ${tmpFile}
...
if [ "${answer}" = "y" ]; then 
  ...
  if [ "${operatingSystem}" = "mingw" ]; then
    ...
    MSYS_NO_PATHCONV=1 ${awsExe} cloudfront create-invalidation --distribution-id ${cloudFrontDistributionId} --paths "/tstool-bitbucket-plugin/latest/doc-user/*" --output json --profile "${awsProfile}" | tee ${tmpFile}
  else
    ${awsExe} cloudfront create-invalidation --distribution-id ${cloudFrontDistributionId} --paths "/tstool-bitbucket-plugin/latest/doc-user/*" --output json --profile "${awsProfile}" | tee ${tmpFile}
  fi
  errorCode=${PIPESTATUS[0]}
  if [ $errorCode -ne 0 ]; then
    logError " "
